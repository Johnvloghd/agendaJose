...
public class AgendaView extends HBox {
    ...
    private DatePicker pickerData = new DatePicker(LocalDate.now()); // CALENDÁRIO
    ...
    public AgendaView(model.Usuario usuarioLogado) {
        this.usuarioAtual = usuarioLogado.getEmail();
        ...
        carregarProfissionais(); // Carrega e já seleciona o primeiro

        Label lblData = new Label("DATA DO ATENDIMENTO (CALENDÁRIO)"); lblData.setStyle(estiloLabel);
        pickerData.setStyle("-fx-control-inner-background: #333333; -fx-text-fill: white; -fx-border-color: #444444;");
        pickerData.getEditor().setStyle("-fx-background-color: #333333; -fx-text-fill: white;");
        ...
        carregarProcedimentos(); // Carrega e já seleciona o primeiro
        ...
        // Se não for ADMIN (ex: José), desativa os botões de configuração
        if (!usuarioLogado.getPerfil().equalsIgnoreCase("ADMIN")) {
            btnAddProcedimento.setDisable(true);
            btnAddBarbeiro.setDisable(true);
            btnAddProcedimento.setStyle(estiloBtnGerenciamento + " -fx-opacity: 0.3; -fx-cursor: default;");
            btnAddBarbeiro.setStyle(estiloBtnGerenciamento + " -fx-opacity: 0.3; -fx-cursor: default;");
        }
    ...
    private void carregarProcedimentos() {
        ...
        if (!procedimentos.isEmpty()) {
            cbProcedimento.setValue(procedimentos.get(0)); // Resolve o bug do campo em branco no carregamento
        }
    }
    ...
    private void carregarProfissionais() {
        ...
        if (!profissionais.isEmpty()) {
            cbProfissional.setValue(profissionais.get(0)); // Resolve o bug do campo em branco no carregamento
        }
    }
    ...
    private void configurarAcoes() {
        ...
        // Ação Adicionar Procedimento (Corrige bug do foco em branco)
        btnAddProcedimento.setOnAction(e -> {
            ...
            dialog.showAndWait().ifPresent(nome -> {
                ...
                if (!nomeLimpo.isEmpty()) {
                    agendaController.cadastrarProcedimento(nomeLimpo);
                    carregarProcedimentos();
                    cbProcedimento.setValue(nomeLimpo); // Força a exibição imediata do novo item
                    LoggerUtil.registrarAcao(usuarioAtual, "Cadastrou o procedimento: " + nomeLimpo);
                }
            });
        });

        // Ação Adicionar Barbeiro (Corrige bug do foco em branco)
        btnAddBarbeiro.setOnAction(e -> {
            ...
            dialog.showAndWait().ifPresent(nome -> {
                String nomeLimpo = nome.trim();
                if (!nomeLimpo.isEmpty()) {
                    agendaController.cadastrarProfissional(nomeLimpo, nomeLimpo.toLowerCase().replace(" ", "") + "@barbearia.com");
                    carregarProfissionais();
                    cbProfissional.setValue(nomeLimpo); // Força a exibição imediata do novo profissional
                    LoggerUtil.registrarAcao(usuarioAtual, "Cadastrou o profissional: " + nomeLimpo);
                }
            });
        });
    }
